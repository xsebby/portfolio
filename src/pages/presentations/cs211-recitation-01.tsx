...
import { RotationDemo } from "@/components/rotation-demo";
import slides from "@/data/recitation-01.json";
    ...
    >
      ...
      {slide.source && (
        <a
          ...
          rel="noreferrer"
        >
          {slide.sourceLabel ?? "Rutgers iLab instructions"} ↗
        </a>
      )}
    </article>
  );
...
export default function Recitation() {
  ...
  return (
    <div ref={root} className={styles.viewer}>
      <Head>
        <title>Bits, rotations &amp; iLab · CS 211 · sebby</title>
        <meta
          name="description"
          content="CS 211 recitation 01: iLab SSH, bitwise operators, masks, and left and right rotations. Includes an interactive byte demo."
        />
        <link
          rel="canonical"
          href="https://sebby.dev/presentations/cs211-recitation-01"
        />
      </Head>
      <header className={styles.toolbar}>
        <Link href="/presentations">← Presentations</Link>
        <div className={styles.viewerTools}>
          <button type="button" aria-pressed={demo} onClick={openDemo}>
            Rotation demo
          </button>
          <button
            type="button"
      ...
      <footer className={styles.controls}>
        ...
        <div className={styles.paging}>
          <button
            ...
            disabled={index === 0 && !demo && !reading}
            onClick={() => {
              setReading(false);
              go(demo ? index : index - 1);
            }}
          >
            ←
          </button>
          <span aria-live="polite">
            {demo ? "Demo" : `${index + 1} / ${slides.length}`}
          </span>
          <button
            type="button"
            aria-label="Next slide"
            disabled={index === slides.length - 1 && !demo && !reading}
            onClick={() => {
              setReading(false);
              go(demo ? index : index + 1);
            }}
